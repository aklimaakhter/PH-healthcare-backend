...
const loginUser = async (payload: ILoginUserPayload) => {
	...
	if (user.password === null && user.googleId !== null) {
		throw new Error(
			"User already has account registered with Google. Please try to login with Google ",
		);
	}
...
const googleLogin = async (payload: IGoogleLoginPayload) => {
	...
	const ifPatientExitWithGoogleAuth = await prisma.user.findUnique({
		where: {
	...
	let user = ifPatientExitWithGoogleAuth;

	if (!ifPatientExitWithGoogleAuth) {
